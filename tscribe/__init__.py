...
def calculate_confidence_statistics(data: dict) -> dict:
    ...
    for item in data["results"]["items"]:
        if item["type"] == "pronunciation":
            ...
            confidence_decimal = float(item["alternatives"][0].get("confidence", DEFAULT_CONF))
...
def decode_transcript_to_dataframe(data: str):
        ...
        for segment in data["results"]["speaker_labels"]["segments"]:
            ...
            if len(segment["items"]) > 0:
                ...
                for word in segment["items"]:
                    ...
                    result = sorted(
                        word_result[-1]["alternatives"], key=lambda x: x.get("confidence", DEFAULT_CONF)
                    )[-1]
    ...
    # If channel identification
        ...
        for word in data["results"]["items"]:
            ...
            if (
                ...
                current_word = sorted(
                    word["alternatives"], key=lambda x: x.get("confidence", DEFAULT_CONF)
                )[-1]
    ...
    # Neither speaker nor channel identification
        ...
        for word in data["results"]["items"]:
            ...
            # Get the word with the highest confidence
            result = sorted(word["alternatives"], key=lambda x: x.get("confidence", DEFAULT_CONF))[-1]
        ...
        for segment in data["results"]["speaker_labels"]["segments"]:
            ...
            if len(segment["items"]) > 0:
                ...
                for word in segment["items"]:
                    ...
                    if float(result.get("confidence", DEFAULT_CONF)) < threshold_for_grey:
                        font = run.font
    ...
    # If channel identification
        ...
        for word in data["results"]["items"]:
            ...
            if table.cell(-1, 1).text == channel:
                ...
                if float(current_word.get("confidence", DEFAULT_CONF)) < threshold_for_grey:
                    font = run.font
                    font.color.rgb = RGBColor(204, 204, 204)
